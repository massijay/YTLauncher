...
namespace YTLauncher
{
    ...
    sealed partial class App : Application
    {
        ...
        {
            Task.Run(() => LaunchAndRun(args));
            base.OnActivated(args);
        }

        private async Task LaunchAndRun(IActivatedEventArgs args)
        {
            ...
            if (args.Kind == ActivationKind.Protocol)
            {
                ...
                Regex matchId = new Regex(@"(?<=youtube(?:-nocookie)?\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})");
                string Id = matchId.Match(protocolArgs.Uri.OriginalString).Value;
                if (String.IsNullOrEmpty(Id) || String.Equals(Id, "videoseries"))
                {
                    var options = new Windows.System.LauncherOptions();
                    options.TargetApplicationPackageFamilyName = "Microsoft.MicrosoftEdge_8wekyb3d8bbwe";
                    await Windows.System.Launcher.LaunchUriAsync(new Uri(protocolArgs.Uri.AbsoluteUri), options);
                }
                else
                {
                    string uri = String.Format(protocolUri, Id);
